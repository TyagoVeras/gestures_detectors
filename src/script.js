  ...
  const estimateHands = async () => {
    ...
    for (const hand of hands) {
      ...
      if (est.gestures.length > 0) {
        ...
        for( const gest of est.gestures){
          window.document.body.style.backgroundColor = colors[gest.name]
        }
        for( const gest of est.gestures){
          // window.document.style.zoom = gest.name === 'paper' ? '1.5%' : '1%'
          // console.log(gest.name);
          document.body.style.transform = gest.name === 'paper' ? 'scale(1.5)' : 'scale(1)'
        }
